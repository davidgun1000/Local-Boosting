function [init_mu,init_T] = initialisation_global_update_used(num_states,num_param,mix_Mu,mix_T,mix_Weights,indx,indx_diag,c,prior,y,num_some,dim_states,x,indx_col)
    ...
    theta_G = theta(dim_states*num_states+1:dim_states*num_states+num_param,1);
    ...
    for j=1:num_states
        ...
        [mean_cond_randeffect_j,chol_cond_randeffect_j] = calculate_condmeanvar_mixture(num_component,mix_Mu, mix_T,num_states,num_param,j,theta_G,dim_states,indx_col);
        [thetaSampled_latent1,thetaSampled_latent2,thetaSampled_latent3,thetaSampled_latent4] = generate_samples(prior,mix_Mu,mix_T,mix_Weights,num_samples,num_some,j,dim_states);
        [~, logSampDensPerComp_latent]=SampleFromMixture(logMixWeights_G,mean_cond_randeffect_j,chol_cond_randeffect_j,num_samples,0,[thetaSampled_latent1;thetaSampled_latent2;thetaSampled_latent3;thetaSampled_latent4]);
        ...
        variance{j,1}(s1,s2) = var(log_weight_alpha);
    else
        [logSampDensPerComp_G] = obtain_logSampDensPerComp_G(mix_Mu,mix_T,num_states,num_param,theta_G,num_component,dim_states);        
        logRBindicator_G_states = logSampDensPerComp_G + logMixWeights';
        max_log_v_G_states = max(logRBindicator_G_states, [], 1);
        logTotalSampDens_G_states = log1p(sum(exp(logRBindicator_G_states - max_log_v_G_states),1)-1) + max_log_v_G_states;
        logRBindicator_G_states = logRBindicator_G_states - repmat(logTotalSampDens_G_states,num_component,1);
        mix_Weights_G_states = exp(logRBindicator_G_states)';
        logMixWeights_G_states = log(mix_Weights_G_states);
    
        [mean_cond_randeffect_condjplusone, chol_cond_randeffect_condjplusone]= calculate_condmeanvar_mixture_condplusone(num_component,mix_Mu,mix_T,num_states,num_param,j,...
            theta_G,[theta_states1(j+1,1);theta_states2(j+1,1);theta_states3(j+1,1);theta_states4(j+1,1)],dim_states,indx_col);
        [thetaSampled_latent1,thetaSampled_latent2,thetaSampled_latent3,thetaSampled_latent4] = generate_samples(prior,mix_Mu,mix_T,mix_Weights,num_samples,num_some,j,dim_states);
        [~, logSampDensPerComp_latent_condplusone]=SampleFromMixture(logMixWeights_G_states,mean_cond_randeffect_condjplusone,chol_cond_randeffect_condjplusone,num_samples,0,[thetaSampled_latent1;thetaSampled_latent2;
            thetaSampled_latent3;thetaSampled_latent4]);
        [~,logTotalSampDens_latent_condplusone]=CombineMixtureComponents(logMixWeights_G_states,logSampDensPerComp_latent_condplusone);
        
        [log_posterior] = calculate_log_posterior(prior,num_samples,num_some,[thetaSampled_latent1;thetaSampled_latent2;thetaSampled_latent3;thetaSampled_latent4],j,...
    ...
    for j=1:num_states
        id_init = find(variance{j,1} == min(min(variance{j,1})));
        init_mu(dim_states*j,1) = grid_com(id_init,1);
        init_mu(dim_states*j-1,1) = grid_com(id_init,1);
        init_mu(dim_states*j-2,1) = grid_com(id_init,1);
        init_mu(dim_states*j-3,1) = grid_com(id_init,1);
        
        init_T(indx_col.I(indx_col.par.Lii{j,1}(1)),1) = grid_com(id_init,2);
        init_T(indx_col.I(indx_col.par.Lii{j,1}(5)),1) = grid_com(id_init,2);
        init_T(indx_col.I(indx_col.par.Lii{j,1}(8)),1) = grid_com(id_init,2);
        init_T(indx_col.I(indx_col.par.Lii{j,1}(10)),1) = grid_com(id_init,2);
