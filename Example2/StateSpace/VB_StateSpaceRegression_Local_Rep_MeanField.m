...
prior.a = 0.5;
prior.b = 0.5;
%y =y_used_demean(1,1:length_data)';
...
for c=1:nrComponent
    ...
    parfor s=1:num_samples
            ...
            [log_posterior,grad_log_posterior] = obtain_grad_param_statespace_sv_Z_higherdim(theta_param(dim_states*num_states+1:dim_states*num_states+num_param,1),theta_param(1:dim_states*num_states,1),y,num_param,...
                num_states,prior,num_some,dim_states,x);            
            ...
            grad_T = temp_mat(sub2ind(size(temp_mat),indx_diag(:,1),indx_diag(:,2)));
    ...
    %stopping rule 
       ...
       mix_T_das{c,1} = mix_T_das{c-1,1};
        ...
        for l = 1:length(id_mean_update_latent)
               id_T_update = [id_T_update;id_mean_update_latent(l,1)*dim_states-1;id_mean_update_latent(l,1)*dim_states];
        end
        id_T_update = [id_T_update;dim_states*num_states+1];
        ...
        for j=1:length(id_mean_update)
            mix_Mu{c,1}(id_mean_update(j,1),1) = init_mu(j,1);   
        end
        ...
        for j=1:length(id_mean_update_latent)
            ...
            if id_mean_update_latent(j) == 1
               mix_T{c,1}(id_mean_update_latent(j),id_mean_update_latent(j))=init_T(j,1); 
               mix_T_das{c,1}(id_mean_update_latent(j),id_mean_update_latent(j)) = log(init_T(j,1));
               
               mix_T{c,1}(id_mean_update_latent(j)+num_states,id_mean_update_latent(j)+num_states)=init_T(j,1); 
               mix_T_das{c,1}(id_mean_update_latent(j)+num_states,id_mean_update_latent(j)+num_states) = log(init_T(j,1));
        ...
        parfor s=1:num_samples
            [lpDens1(s,1),grad1(:,s)] = obtain_grad_param_statespace_sv_Z_higherdim(thetaSampled1(dim_states*num_states+1:dim_states*num_states+num_param,s),...
                thetaSampled1(1:dim_states*num_states,s),y,num_param,num_states,prior,num_some,dim_states,x);           
...
%             
    ...
    if c<nrComponent
       ...
       %else
           %tic
           [id_mean_update_latent,init_mu_latent,init_T]=initialisation_local_update_used(num_states,num_param,mix_Mu,mix_T,mix_Weights,indx,indx_diag,num_chosen,c,prior,y,num_some,dim_states,x,indx_col);
                     %move=2;
           %toc
     ...
     mix_Weights_old=mix_Weights;
      mod_name=['mixCop_Local',num2str(c),'_',num2str(num_some),'_statespace_sv_Z_regression_MF.mat'];
    save(mod_name,'mix_Weights','mix_Mu','mix_T','mix_T_das','LB_est','mix_Weights_old','id_mean_update_store','measure_latent','LB_smooth');
