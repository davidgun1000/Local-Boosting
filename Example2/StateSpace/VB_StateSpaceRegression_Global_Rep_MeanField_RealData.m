...
%distribution for the prior for state vectors. Diagonal precision matrix is
%used for each mixture component.
...
indx = indx_diag; % finding index of the latent states
...
for c=1:nrComponent
    ...
    cpu_time(c,1) = toc;
        ...
        %initialisation of variational parameters
        [mix_Mu{c,1}(1:dim_states*num_states,1),init_T] = initialisation_global_update_used(num_states,num_param,mix_Mu,mix_T,mix_Weights,indx,indx_diag,...
            c,prior,y,num_some,dim_states,[x1,x2,x3],indx_col);
        %mix_Mu{c,1}(num_states+1:num_states+num_param,1) = initialisation_global_parameter(num_states,num_param,mix_Mu(1:c-1),mix_T(1:c-1),mix_Weights,...
        %    indx,indx_diag,num_chosen,c,prior,y,num_some);
        mix_Mu{c,1}(dim_states*num_states+1:dim_states*num_states+num_param,1) = mix_Mu{c-1,1}(dim_states*num_states+1:dim_states*num_states+num_param,1);
    ...
    mod_name=['mixCop_Global',num2str(c),'_',num2str(num_some),'_statespace_Z_regression_FF_MF_RealData_Food.mat'];
    save(mod_name,'mix_Weights','mix_Mu','mix_T','mix_T_das','LB_est','mix_Weights_old','measure_latent','LB_smooth','cpu_time','prior');
