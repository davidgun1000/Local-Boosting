...
% hp_sig2=100;
% eta=x*beta;
% grad_log_likelihood=x'*y(:,1)-x'*(exp(eta)./(1+exp(eta)));
% grad_log_prior=-beta./hp_sig2;
...
[~, logZ, dlogZ, ~] = zpdf_general(alpha_complex, prior.mean, prior.s, prior.a, prior.b);
...
grad_prior_alpha_complex = dlogZ;

% const_alpha_complex = prior.w_a(1)*t_pdf(alpha_complex,prior.mu(1),sig_alpha(1),prior.df) + prior.w_a(2)*t_pdf(alpha_complex,prior.mu(2),sig_alpha(2),prior.df);
% 
...
%grad_prior_alpha_complex = -1./cons_alpha_complex.*( prior.w_a(1)*normpdf(alpha_complex,prior.mu(1),sig_alpha(1)).*((alpha_complex-prior.mu(1))./prior.sig2_alpha(1)) + prior.w_a(2)*normpdf(alpha_complex,prior.mu(2),sig_alpha(2)).*((alpha_complex-prior.mu(2))./prior.sig2_alpha(2)));
...
%grad_log_likelihood_alpha = y(:,1)-(exp(eta)./(1+exp(eta)));
...
prior_alpha_complex = sum(logZ);

%prior_alpha_complex = sum(log(prior.w_a(1)*normpdf(alpha_complex,prior.mu(1),sig_alpha(1)) + prior.w_a(2)*normpdf(alpha_complex,prior.mu(2),sig_alpha(2))));

%prior_alpha = sum(log(prior.w_a(1)*normpdf(alpha,prior.mu(1),sig_alpha(1)) + prior.w_a(2)*normpdf(alpha,prior.mu(2),sig_alpha(2))));
